...
class VisualisateurSegmentationRPG_Onyxia:
    ...
    def visualiser_tuile_complete(self, tuile_id, fold='5', figsize=(28, 14)):
        ...
        self._afficher_segmentation_semantique(axes[0], data['semantic_pred'], 
                                              f"Segmentation sémantique - Tuile {tuile_id}")
        
        # 2. RPG seul
        self._afficher_rpg(axes[1], rpg_filtre, coords_info, 
                          f"RPG - Tuile {tuile_id}")
        
        # 3. Superposition sémantique + RPG
        self._afficher_superposition_semantic(axes[2], data['semantic_pred'], rpg_filtre, 
                                            coords_info, f"Sémantique + RPG - Tuile {tuile_id}")
        
        
        # 4. Heatmap de confiance
        if 'confidence_map' in data:
            self._afficher_confidence(axes[3], data['confidence_map'], 
                                    f"Confiance - Tuile {tuile_id}")
        else:
            axes[3].axis('off')
            axes[3].text(0.5, 0.5, 'Pas de données\nde confiance', 
    ...
    def _afficher_segmentation_semantique(self, ax, semantic_pred, titre):
        """Affiche la segmentation sémantique avec légende"""
        # Traitement de la segmentation sémantique
        if len(semantic_pred.shape) == 3:
            # Si c'est un tensor avec des probabilités par classe, prendre l'argmax
            if semantic_pred.shape[0] > 1:
                semantic_map = np.argmax(semantic_pred, axis=0)
            else:
                semantic_map = semantic_pred[0]
        else:
            semantic_map = semantic_pred
    ...
    # Ajouter la légende si pas trop d'éléments
        if len(legend_elements) <= 10:
            ax.legend(handles=legend_elements,
                 ...
                 fontsize=7,
                 title='Instances + RPG')
    def _afficher_superposition_semantic(self, ax, semantic_pred, rpg_filtre, coords_info, titre):
...
def exemple_utilisation():
    ...
    SOUS_DOSSIER = "vergers-france/preds_sem_128_10"
...
def test_rapide_s3():
    ...
    files = fs.glob(f"{bucket_name}/vergers-france/preds_sem_128_10/*_best_fold_*.npy")
